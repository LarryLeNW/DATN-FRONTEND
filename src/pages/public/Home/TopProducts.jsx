...
const TopProducts = () => {
    ...
    <div className="md:w-1/5 w-full md:sticky md:top-20 bg-white p-4 md:p-6 rounded-lg shadow-xl space-y-6 max-h-screen overflow-y-auto">
        <h2 className="text-2xl font-semibold text-center text-gray-900">Danh mục</h2>
        
        <div className="space-y-4">
            {/* Hiển thị tất cả danh mục trên màn hình lớn và tối đa 4 danh mục trên màn hình nhỏ */}
            {categories &&
                (showAllCategories || !isMobile ? categories : categories.slice(0, 3)).map((item, index) => (
                    <div
                        key={index}
                    ...
                    >
                        ...
                        <span className="text-gray-800 md:text-lg font-medium">{item.name}</span>
                    </div>
                ))}
            ...
            <div className="md:w-4/5 w-full p-4">
                ...
                <h1 className="text-center text-red-600 text-3xl mt-5 font-semibold">Gợi ý hôm nay</h1>
